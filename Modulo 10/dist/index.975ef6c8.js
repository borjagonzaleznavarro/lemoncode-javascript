...
// swapFirstSecond
const swapFirstSecond = ([firstItem, secondItem, ...restItems])=>newArray = [
        secondItem,
        firstItem,
        ...restItems
    ];
...
// searchInStringV1
const searchInStringV1 = (string, letter)=>string.split("").reduce((sum, character)=>{
        if (character == letter) sum++;
        return sum;
    }, "");
console.log(searchInStringV1("cadena de texto", "a"));
// searchInStringV2
const searchInStringV2 = (string, character)=>string.split("").filter((string)=>string == character).length;
console.log(searchInStringV2("cadena de texto", "a"));
// sortCharacters
const sortCharacters = (string)=>string.split("").sort((a, b)=>a > b ? 1 : -1).join();
...
const subtotal = (cart)=>cart.reduce((sum, product)=>{
        if (product.category == "Droguer\xeda") sum += product.price * product.units;
        return sum;
    }, 0);
...
const orderedList = (cart)=>{
    return cart.sort((a, b)=>a.category > b.category ? 1 : -1).map((product)=>product.product + ": " + product.price * product.units + "€");
};
console.log(orderedList(shoppingCart));

},{}]},["66IEb","8lqZg"], "8lqZg", "parcelRequire94c2")
