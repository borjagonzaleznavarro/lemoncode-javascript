...
onSubmitForm('insert-feature-button', () => {
  featureValidation
    .validateField('newFeature', uploadProperty.newFeature).then((result) => {
      onSetError('newFeature', result);
      if (result.succeeded) {
        onAddFeature(uploadProperty.newFeature);
        uploadProperty.mainFeatures.push(uploadProperty.newFeature);
        formValidation
          .validateField('mainFeatures', uploadProperty.mainFeatures).then((result) => {
            onSetError('mainFeatures', result);
            uploadProperty.mainFeatures.map((feature) => {
              onSubmitForm(formatDeleteFeatureButtonId(feature), () => {
                onRemoveFeature(feature);
                let index = uploadProperty.mainFeatures.indexOf(feature);
...
onUpdateField('equipments', (event) => {
  ...
  //     });
  formValidation.validateField('equipments', uploadProperty.newEquipment).then((result) => {
    onSetError('equipments', result);
    uploadProperty.equipmentIds.push(uploadProperty.newEquipment);
  });
});
